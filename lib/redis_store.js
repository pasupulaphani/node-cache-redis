 ...
 */
const RedisStore = module.exports = function ({name, redisOptions, poolOptions, logger, ttlInSeconds} ={}) {

  this.name = name || `redisStore-${Math.random().toString(36).substr(2, 10)}`;
...
RedisStore.prototype.getset = function (key, value, ttlInSeconds) {
  ...
  return this.sendCommand("getset", [key, value])
    .then(function (value) {
      try {
        value = JSON.parse(value);
      } catch (e) {
      ...
      if (ttlInSeconds) {
        return _that.sendCommand("expire", [key, ttlInSeconds])
          .then(function () {
            return value;
          });
      } else
